...
from .helper import parse_indian_date, _safe
...
class JournalBuilder:
    ...
    def to_journal_entries(self, bills: list[dict]) -> list[JournalEntry]:
        entries = []
        for bill in bills:
            entry = self.to_journal_entry(bill)
            if entry:
    ...
    def close_books(
        ...
        if sales_bals:
            ...
            lines.append(EntryLine(
                TRADING_AC, DrCr.CREDIT, round(sales_total, 2), "Transfer of Sales & Direct Income to Trading A/c",
            ))
            entries.append(JournalEntry(
                date=period_end,
                voucher_type="Journal Voucher",
        ...
        if purchase_bals:
            lines = [
                EntryLine(TRADING_AC, DrCr.DEBIT, round(purchase_total, 2), "Transfer of Purchases & Direct Expenses to Trading A/c"),
            ]
        ...
        if indir_inc_bals:
            ...
            for acc_name, bal in indir_inc_bals.items():
                ...
                lines.append(EntryLine(
                    Account(acc_name, group), DrCr.DEBIT, bal, f"Closing {acc_name} to P&L A/c",
                ))
            lines.append(EntryLine(
                PL_AC, DrCr.CREDIT, round(indir_inc_total, 2), "Transfer of Indirect Income to P&L A/c",
            ))
            entries.append(JournalEntry(
                date=period_end,
                voucher_type="Journal Voucher",
        ...
        if indir_exp_bals:
            lines = [
                EntryLine(PL_AC, DrCr.DEBIT, round(indir_exp_total, 2), "Transfer of Indirect Expenses to P&L A/c"),
            ]
            for acc_name, bal in indir_exp_bals.items():
                acc_obj = gl.get(acc_name)
                group = acc_obj.group if acc_obj else AccountGroup.INDIRECT_EXPENSES
                lines.append(EntryLine(
                    Account(acc_name, group), DrCr.CREDIT, bal, f"Closing {acc_name} to P&L A/c",
                ))
            entries.append(JournalEntry(
                date=period_end,
                voucher_type="Journal Voucher",
...
def to_journal_entry(bill: dict) -> Optional[JournalEntry]:
    return JournalBuilder().to_journal_entry(bill)


def to_journal_entries(bills: list[dict]) -> list[JournalEntry]:
    return JournalBuilder().to_journal_entries(bills)
...
def gst_settlement_entry(
    ...
    return JournalBuilder().gst_settlement_entry(
...
def close_books(
    ...
    return JournalBuilder().close_books(gl, period_end, period_label)
