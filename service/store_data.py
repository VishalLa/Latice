...
class PushEntryPointData:
    ...
    @staticmethod 
    def push_bank_statements(session: Session, statements_data: List[Dict[str, Any]]) -> bool:
        """
        Takes a list of dictionaries (from the loader) and pushes them to the BankStatementModel.
        """
        try:
            # Unpack the dictionaries directly into the SQLAlchemy model
            db_statements = [BankStatementModel(**data) for data in statements_data]
    ...
    @staticmethod 
    def push_ledgers(session: Session, ledgers_data: List[Dict[str, Any]]) -> bool:
        """
        ...
        try:
            ...
            for data in ledgers_data:
                # Ensure the 'source' string is properly cast to the Enum if it exists
                if 'source' in data and isinstance(data['source'], str):
                    data['source'] = LedgerSource(data['source'])
                    
                db_ledgers.append(LedgerFormatModel(**data))
    ...
    @staticmethod
    def push_all_data(session: Session, statements_data: List[Dict[str, Any]], ledgers_data: List[Dict[str, Any]]) -> bool:
        """
