...
class LedgerBuilder:
    ...
    def __init__(
        ...
        self.gl = gl if gl else GeneralLedger()
        self.as_on = as_on or date.today()


    # Pipeline: bills → journal entries → GL → (optional) closing
    @classmethod
    def build(
        cls,
        bills: list[dict],
        ...
        period_label: str = "",
    ) -> tuple["LedgerBuilder", list[JournalEntry], Optional[ClosingResult]]:
        """
        ...
        Args:
            ...
            _prebuilt_entries : TDS-modified entries from the ledger orchestrator.
        ...
        Returns:
            builder         : a LedgerBuilder wrapping the populated GeneralLedger
                              (includes closing entries if requested), with `as_on`
                              set to close_books_on if given, else today.
        ...
        if _prebuilt_entries is not None:
            ...
            tx_entries = JournalBuilder.to_journal_entries(bills)
        ...
        if close_books_on is not None:
            closing_result = JournalBuilder().close_books(gl, close_books_on, period_label)
        ...
        builder = cls(gl=gl, as_on=close_books_on or date.today())
        return builder, all_entries, closing_result


    def trial_balance(self) -> TrialBalance:
        lines = []

        for ledger_acc in self.gl.accounts:
    ...
    def gst_summary(self, period_label: str = "") -> GSTSummary:
        """
        ...
        # (remaining_igst_itc is only ever nonzero here once net_igst has
        # already hit 0 — see step 1 — so capping directly by out_cgst is
        # correct with no extra conditional needed.)
    ...
    def extract_cash_book(self) -> list[CashBookLine]:
        cash_acc = self.gl.get(COA.CASH.name)
        bank_acc = self.gl.get(COA.BANK.name)
    ...
    def creditors_ageing(self) -> list[AgeingLine]:
        result = []

        for acc in self.gl.accounts_in_group(AccountGroup.SUNDRY_CREDITORS):
            ...
            for posting in acc.postings:
                days = (self.as_on - posting.date).days
...
def build_ledger(
    ...
    builder, all_entries, closing_result = LedgerBuilder.build(
        ...
        _prebuilt_entries=_prebuilt_entries, 
        close_books_on=close_books_on, 
        period_label=period_label,
    )
    return builder.gl, all_entries, closing_result
