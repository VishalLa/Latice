...
class EntryLine:
    ...
    @classmethod
    def _enforce_positive_amount(cls, v: float) -> float:
        if v < 0:
            raise ValueError(f"EntryLine amount must be positive, got {v}")
        return v


class JournalEntry(SchemaBase):
    ...
    def _validate_double_entry(self) -> "JournalEntry":
        ...
        debits  = round(sum(l.amount for l in self.lines if l.dr_cr == DrCr.DEBIT))
        credits = round(sum(l.amount for l in self.lines if l.dr_cr == DrCr.CREDIT))

        if abs(debits - credits) > TOLARANCE:
            raise ValueError(
