...
def _enum_or_str_value(v: Any) -> str:
    """
    `source` shows up in two shapes depending on which code path built the
    record: DB-model reads unwrap it to a plain string ("auto"/"manual")
    before handing it to the schema object, while records fresh off the
    loader/dataclass keep it as a LedgerSource enum instance. Handle both
    instead of assuming one, so this doesn't crash when the value has
    already been unwrapped upstream.
    """
    if v is None:
        return ""
...
def _ledger_row(rec: LedgerFormat) -> tuple:
    ...
    return (
        ...
        amount,
        direction,
        _enum_or_str_value(getattr(rec, "source", None)),
    )
