...
from .ai_matcher import (
    ...
    _get_bank_direction_amount,
    _get_gl_direction_amount,
    _safe_parse_json,
    get_shared_llm,
)
...
def _classify_one(
    ...
    if is_bank:
        amt, direction = _get_bank_direction_amount(item)
        text = _safe_str(item, "narration")
    else:
        amt, direction = _get_gl_direction_amount(item)
    ...
    for cp in counterpart_pool:
        if is_bank:
            cp_amt, cp_dir = _get_gl_direction_amount(cp)
        else:
            cp_amt, cp_dir = _get_bank_direction_amount(cp)
...
def _resolve_timing_candidates(
    ...
    for gl_entry in list(ledger_pool):
        ...
        for bank_entry in bank_pool:
            ...
            bk_amt, bk_dir = _get_bank_direction_amount(bank)
        ...
        found = (bank_entry, diff, ambiguous, len(candidates))

        if found:
            bank_entry, diff, ambiguous, n_candidates = found
            bank = bank_entry["item"]
            matches.append({
...
def _resolve_split_candidates(
    ...
    def _prefilter(pool: List[Dict[str, Any]], target: float, target_dir: str, is_bank: bool) -> List[Tuple[Any, float]]:
        pairs = []
        for entry in pool:
            item = entry["item"]
            amt, item_dir = _get_bank_direction_amount(item) if is_bank else _get_gl_direction_amount(item)
            if amt <= 0:
                continue
    ...
    for bank_entry in list(bank_targets):
        if not _identity_contains(bank_pool, bank_entry):
            continue  
        bank = bank_entry["item"]
        b_amt, b_dir = _get_bank_direction_amount(bank)
        if b_amt <= 0:
            continue
        candidates = _prefilter(ledger_pool, b_amt, b_dir, is_bank=False)
    ...
    for gl_entry in list(ledger_targets):
        if not _identity_contains(ledger_pool, gl_entry):
            continue  
        gl = gl_entry["item"]
        g_amt, g_dir = _get_gl_direction_amount(gl)
        if g_amt <= 0:
            continue
        candidates = _prefilter(bank_pool, g_amt, g_dir, is_bank=True)
        candidates = FuzzyMatcher._closest_candidates(
...
class DraftAccountSuggestion(BaseModel):
...
def _generate_journal_drafts(
    ...
    for entry in missing_bank:
        bank = entry["item"]
        amt, direction = _get_bank_direction_amount(bank)
    ...
    return drafts, still_unresolved, not llm_available, (None if llm_available else skip_reason)
...
def _build_review_queue(
    ...
    for entry in entries:
        ...
        queue.append({
            ...
            "amount": evidence.get("amount", _get_bank_direction_amount(item)[0] if is_bank else _get_gl_direction_amount(item)[0]),
